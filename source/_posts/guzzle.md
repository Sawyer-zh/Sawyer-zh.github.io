...
// 到这里可以看到:
// 1.$handler 实际上是个闭包 :
//       接受两个参数($request,$options)
//       调用这个闭包相当于调用$default , 或者$sync 
// 2.$default , $sync 是什么?
//       new CurlMultiHandler , new CurlHandler 
// 3.$config['handler'] 是 HandlerStack 对象 ,这个对象里面$handler是1 .里面还有一个stack,保存了Middleware Callable
...
// 这里可以看出:
//    1.调用$default 实际上调用 CurlMultiHandler 的 __invoke方法 返回一个$promise
//    2.调用$sync 实际上调用 CurlHandler 的 __invoke方法 直接执行了curl_exec ,返回一个FulfilledPromise
//    3.CurlMultiHandler , CurlHandler 实例化的时候都用到了 CurlFactory . 
...
//到这里可以得出结论:
//      1. EasyHandle 实际上把一个请求和结果包裹起来了
//      2. CurlFactory::create 中有curl_init方法 
...
// 到这里就可以发现同步请求的流程了
//    1. __call方法重载,最终会调用$handler($request,$options) 这个由上面分析 返回的是一个fulfilledpromise
//    2. 在这里promise_for , wait 的作用, 其实就是直接返回
//    3. 使用curl发送请求,CurlFactory, CurlHandler(同步) , CurlMultiHandler(异步),底层都依赖CurlFactory
//    4. 异步返回的结果是promise,同步则返回请求的结果因为他调用了wait()方法
